...
def send_telegram_top10_and_csv(
    ...
    """Top 10 સ્ટોક્સને સુંદર કેપ્શન તરીકે મોકલશે અને બાકીના સ્ટોક્સની CSV અટેચ કરશે."""
    if not bot_token or not chat_id:
        print("⚠️ ટેલિગ્રામ બોટ ટોકન અથવા ચેટ આઈડી મળેલ નથી.")
        return False
    ...
    # 2. Top 10 માટે મેસેજ/કેપ્શન તૈયાર કરો
    caption = f"🎯 *12-Marks Advanced Weekly Screener Report*\n"
    caption += f"📊 Total Candidates Found: *{len(passed_stocks)}*\n\n"
    caption += f"🔥 *TOP {len(top_10_stocks)} STRONGEST CANDIDATES:* 🔥\n"
    caption += "───────────────────────────\n"
    ...
    for idx, st in enumerate(top_10_stocks, 1):
        ...
        caption += f"• Price: ₹{st['close']} | RSI: {st['rsi']} | IV: ₹{st['iv']}\n"
    ...
    if remaining_stocks:
        ...
        try:
            ...
            if res_data.get("ok"):
                print("✅ Top 10 Alert અને બાકીના સ્ટોક્સની CSV ફાઈલ સફળતાપૂર્વક મોકલાઈ ગઈ.")
                return True
            else:
                print(
...
def main():
    ...
    if not passed_stocks:
        ...
        # નવા નવતર ફંક્શન દ્વારા Top 10 + બાકીના સ્ટોક્સની CSV મોકલો
