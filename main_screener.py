...
def load_config(config_path="config.yml"):
    if not os.path.exists(config_path):
        raise FileNotFoundError(f"કન્ફિગરેશન ફાઈલ '{config_path}' મળી નથી.")
...
# ==============================================================================
# 3. NATIVE TECHNICAL INDICATORS (NO EXTERNAL 'ta' LIBRARY NEEDED)
# ==============================================================================
...
# ==============================================================================
# 4. TECHNICAL & FUNDAMENTAL ANALYSIS (11 MARKS)
# ==============================================================================
def analyze_stock(ticker_symbol, config):
    try:
        ...
        df['EMA_200'] = calculate_ema(df['Close'], tc['ema_200']) if len(df) >= 200 else df['EMA_55']
        ...
        if ema_spread <= tc['junction_threshold']:
            score += 1
            reasons.append("EMA Junction")
            
        # 3. Doji Reversal at EMA 4
        ...
        if tc['rsi_min'] <= latest['RSI'] <= tc['rsi_max']:
            score += 1
            reasons.append(f"Strict RSI ({latest['RSI']:.1f})")
            
        # 5. Fibonacci Support Zone
...
def main():
    ...
    start_msg = f"🔍 *11-Marks Stock Screener Started*\nScrutinizing total *{total_stocks}* stocks..."
    ...
    print("\n==================================================")
    print(f"✅ Screening Finished. Candidates: {len(passed_stocks)}")
    print("==================================================\n")
    
    if not passed_stocks:
        nil_message = (
            "📊 *11-Marks Stock Screener Report*\n\n"
            "❌ *Result:* No stocks met criteria today (0 Stocks Found).\n\n"
